...
<div class="container py-4">
    ...
    <div class="table-responsive shadow-sm">
        <div class="d-flex justify-content-between align-items-center mb-3 px-2">
            <div>
                <span class="text-muted">Total : <strong>{{ $clients->count() }}</strong> clients</span>
            </div>
            <form method="GET" action="{{ route('clients.index') }}" class="d-flex" role="search">
        ...
        @if(request('search'))
            <div class="mb-3 px-2">
                ...
                <!-- Bouton de retour à la liste complète, inspiré du design -->
                <a href="{{ route('clients.index') }}" class="btn btn-sm btn-secondary ms-3">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <a href="{{ route('clients.show', $client) }}" class="btn btn-sm btn-info me-1" title="Voir">
                            <i class="bi bi-eye-fill"></i>
                        </a>

                        @if(auth()->user()->role === 'admin')
                            <a href="{{ route('clients.edit', $client) }}" class="btn btn-sm btn-warning me-1" title="Modifier">
...
<script>
    document.getElementById('deleteModal').addEventListener('show.bs.modal', function (event) {
        ...
        // Mettre à jour le message avec le nom du client
        modal.querySelector('.modal-body p.fs-5').textContent = 
            `Êtes-vous sûr de vouloir supprimer le client "${clientName}" ?`;
        
        // Mettre à jour l'action du formulaire
        var deleteForm = document.getElementById('deleteForm');
        deleteForm.action = "{{ route('clients.index') }}/" + clientId;
    });
</script>
@endsection
