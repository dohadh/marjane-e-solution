...
<div class="container py-4">
    ...
    @if(request('search') && $users->isEmpty())
        <div class="alert alert-warning">
            Aucun utilisateur trouvé pour la recherche "{{ request('search') }}"
        </div>
    @endif
    ...
    <div class="table-responsive shadow-sm">
        <div class="d-flex justify-content-between align-items-center mb-3 px-2">
            <div>
                <span class="text-muted">Total : <strong>{{ $users->count() }}</strong> utilisateurs</span>
            </div>
        ...
        @if(request('search'))
            <div class="mb-3 px-2">
                ...
                <a href="{{ route('users.index') }}" class="btn btn-sm btn-secondary ms-3">
                ...
                @forelse ($users as $user)
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            <a href="{{ route('users.show', $user) }}" class="btn btn-sm btn-info me-1" title="Voir">
                                <i class="bi bi-eye-fill"></i>
                            </a>

                                <a href="{{ route('users.edit', $user) }}" class="btn btn-sm btn-warning me-1" title="Modifier">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (deleteModal) {
            deleteModal.addEventListener('show.bs.modal', function(event) {
                ...
                // Use Laravel route helper for reliability
                deleteForm.action = "{{ route('users.destroy', '') }}/" + userId;
                
                // Update confirmation message with user name
                const confirmationMessage = deleteModal.querySelector('.confirmation-message');
                if (confirmationMessage && userName) {
                ...
                if (modalTitle) {
                    modalTitle.textContent = `Supprimer ${userName}`;
                }
            });
        }
    });
